...
ADNS2620::ADNS2620(int sda1, int sda2, int scl)
{
	_sda1 = sda1;
	_sda2 = sda2;
	_scl = scl;
...
char ADNS2620::read(char address)
{
	...
	pinMode(_sda2, OUTPUT); //Make sure the SDIO pin is set as an output.
    ...
    pinMode(_sda2, INPUT);	//Make sure the SDIO pin is set as an input.
	digitalWrite(_sda1, HIGH); //Enable the internal pull-up
	digitalWrite(_sda2, HIGH); //Enable the internal pull-up
    ...
    for(int value_bit=7; value_bit >= 0; value_bit--){
		...
		if(digitalRead(_sda1))value[0] |= (1<<value_bit);
		if(digitalRead(_sda2))value[1] |= (1<<value_bit);
